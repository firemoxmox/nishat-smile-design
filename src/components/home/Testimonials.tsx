    ...
    <div className="container mx-auto px-4">
      <div className="text-center mb-14">
        ...
        <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold font-display text-foreground mb-4">
          Real Patients, <span className="text-primary">Real Results</span>
        </h2>
        <p className="text-muted-foreground font-body max-w-xl mx-auto text-sm">
          Hear from the people who trust us with their smiles every day.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        {reviews.map((r) => (
          <div
            key={r.name}
            className="relative bg-card rounded-xl p-6 border border-border/50 hover:shadow-lg hover:shadow-primary/5 transition-all duration-300"
          >
            <Quote className="w-7 h-7 text-primary/10 absolute top-5 right-5" />
            ...
            <div className="flex items-center gap-3 mt-auto">
              <div className="w-9 h-9 rounded-full bg-gradient-to-br from-primary to-accent flex items-center justify-center text-white font-bold text-xs font-display">
                {r.name[0]}
              </div>
              <div>
